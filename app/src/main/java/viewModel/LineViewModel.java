...
public class LineViewModel extends ViewModel {
    ...
    public void initForecastChart() {
        ...
        //线
        for (int i = 0; i < WebConnect.numOfForecastLines; i++) {
            //绑定数据
            List<PointValue> tempArrayList = new ArrayList<>();//一条线的数据
            for (int p = 0; p < WebConnect.numOfForecastPoints; p++) {
                int x = p + WebConnect.numOfRealPoints;//预测点接在真实后面
                float y = WebConnect.getLineData().get(i + WebConnect.numOfRealLines)[p];
...
//            line.setPointColor(Color.rgb(255, 255, 255));//点的颜色 白色
            ...
            if (lines.size() >= WebConnect.numOfRealLines + WebConnect.numOfForecastLines) {
                //如果已经有这条线了
                lines.set(i + WebConnect.numOfRealLines, line);
            } else {
                lines.add(line);
            }
        }
        ...
        for (int i = 0; i < WebConnect.numOfForecastLines; i++) {
            ...
            int day = 0;
            for (int j = 0; j < WebConnect.numOfForecastPoints; j++) {
                day++;
                if (day > 30) {
                    day %= 30;
                }
        ...
        for (int i = 0; i < WebConnect.numOfForecastLines; i++) {
            ...
            //每个点，包括前面真实和后面预测
            for (int p = 0; p < WebConnect.numOfRealPoints + WebConnect.numOfForecastPoints; p++) {
                AxisValue valueX = new AxisValue(p);//这里的数字是坐标的数值，比如第一个坐标就是0
                //将坐标的数值和对应的文字标签绑定起来
                //分为前（真实）后（预测）
                if (p < WebConnect.numOfRealPoints)
                    //前，是真实线0的标签
                    valueX.setLabel(axisLableList.get(0).get(p));
                else
                //后，是预测线i的标签，此时的点值应该剪掉前面的真实点总数
                {
                    int thisLineIndex = i + WebConnect.numOfRealLines;
                    int thisP = p - WebConnect.numOfRealPoints;
    ...
    public void initRealChart() {
        ...
        WebConnect.initReal();
        //线
        for (int i = 0; i < WebConnect.numOfRealLines; i++) {
            List<PointValue> tempArrayList = new ArrayList<>();//一条线的数据
            for (int j = 0; j < WebConnect.numOfRealPoints; j++) {
        ...
        //建立标签
        for (int i = 0; i < WebConnect.numOfRealLines; i++) {
            List<String> strings = new ArrayList<>(WebConnect.numOfRealPoints);
            int day = 0;
            for (int j = 0; j < WebConnect.numOfRealPoints; j++) {
                day++;
                if (day > 30) {
    ...
    public int getNumOfRealLines() {
        return WebConnect.numOfRealLines;
    }
