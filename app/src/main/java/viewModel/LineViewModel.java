...
//            line.setPointColor(Color.rgb(255, 255, 255));//点的颜色 白色
        ...
        for (int i = 0; i < Repository.numOfForecastLines; i++) {
            ...
            int day = 0;
            for (int j = 0; j < Repository.numOfForecastPoints; j++) {
                day++;
                if (day > 30) {
                    day %= 30;
                }
        ...
        for (int i = 0; i < Repository.numOfForecastLines; i++) {
            ...
            //每个点，包括前面真实和后面预测
            for (int p = 0; p < Repository.numOfRealPoints + Repository.numOfForecastPoints; p++) {
                AxisValue valueX = new AxisValue(p);//这里的数字是坐标的数值，比如第一个坐标就是0
                //将坐标的数值和对应的文字标签绑定起来
                //分为前（真实）后（预测）
                if (p < Repository.numOfRealPoints)
                    //前，是真实线0的标签
                    valueX.setLabel(axisLableList.get(0).get(p));
                else
                //后，是预测线i的标签，此时的点值应该剪掉前面的真实点总数
                {
                    int thisLineIndex = i + Repository.numOfRealLines;
                    int thisP = p - Repository.numOfRealPoints;
    ...
    public void initRealChart() {
        ...
        //建立标签
        for (int i = 0; i < Repository.numOfRealLines; i++) {
            List<String> strings = new ArrayList<>(Repository.numOfRealPoints);
            int day = 0;
            for (int j = 0; j < Repository.numOfRealPoints; j++) {
                day++;
                if (day > 30) {
    ...
    public int getNumOfRealLines() {
        return Repository.numOfRealLines;
    }
